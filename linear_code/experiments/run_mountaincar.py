...
from replearn import mountaincar
...
def main(argv):
    ...
    obs_encoder = features.RBFEncoder(
        centers=features.uniform_centers(env, 10),
        scales=features.normalized_scales(env, 0.15),
        normalized=False,
    )
    action_encoder = features.OneHot(3)
    history_encoder = features.TruncatedHistoryEncoder(1)
    ...
    for i, seed in enumerate(SEEDS):
        ...
        s_t, a_t, s_tp1 = rollout.rollout_dataset(
            ...
            env_cls=mountaincar.MountainCar,
            policy=mountaincar.MountainCarPolicy(None, 0.1),
    ...
    with open(os.path.join(FLAGS.results_dir, "mountaincar.pkl"), "wb") as f:
