...
class ReplayMemory:
    ...
    def sample(self, n):
        idxes = np.random.randint(
            0, self.buffer_limit if self.full else self.idx, size=n
        )
        return (
            self.observation[idxes],
            self.action[idxes],
            self.reward[idxes],
            self.next_observation[idxes],
            self.terminal[idxes],
        )
    ...
    def sample_seq(self, seq_len, batch_size):
        n = batch_size
        l = seq_len
        obs, act, rew, next_obs, term = self._retrieve_batch(
    ...
    def sample_probe_data(self, data_size):
        idxes = np.random.randint(
            0, self.buffer_limit if self.full else self.idx, size=data_size
        )
    ...
    def _sample_idx(self, L):
        ...
        while not valid_idx:
            ...
            valid_idx = (not self.idx in idxs[1:]) and (
                not self.terminal[idxs[:-1]].any()
            )
        return idxs
    ...
    def _retrieve_batch(self, idxs, n, l):
        ...
        return (
            self.observation[vec_idxs].reshape(l, n, -1),
            self.action[vec_idxs].reshape(l, n, -1),
            self.reward[vec_idxs].reshape(l, n),
            self.next_observation[vec_idxs].reshape(l, n, -1),
